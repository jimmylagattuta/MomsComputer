...
function titleFor(thread: SupportTextThreadSummary) {
  const subject = String(thread.subject || "").trim();
  if (subject) return subject;

  const assigned = String(thread.assigned_agent_name || "").trim();
  if (assigned) return `Support • ${assigned}`;

  return "Text Mom thread";
}

function metaFor(thread: SupportTextThreadSummary) {
  const status = String(thread.status || "open");
  const ts = thread.last_message_at || thread.started_at || thread.created_at;
...
export default function HistoryDrawer({
  ...
  open: boolean;
  onClose: () => void;
  threads: SupportTextThreadSummary[];
  onSelectThread: (threadId: number) => void;
  onUpdateThreads: (rows: SupportTextThreadSummary[]) => void;
}) {
  ...
  useEffect(() => {
      ...
      if (prev.length > 0) {
        debounceRef.current = setTimeout(async () => {
          ...
          try {
            ...
            onUpdateThreads(rows);
          } catch (e: any) {
            setSearchError(
              e?.message ? String(e.message) : "Failed to load threads"
            );
          } finally {
            setSearching(false);
          }
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const rows = await fetchSupportTextThreads(q);
        onUpdateThreads(rows);
      } catch (e: any) {
  ...
  }, [query, open, onUpdateThreads]);

  if (!visible) return null;

  const empty = !threads || threads.length === 0;
        ...
        >
          ...
          <View style={styles.searchWrap}>
            <TextInput
              ...
              placeholder="Search inside text threads…"
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          {query.trim().length === 0 && empty ? (
            <Text style={styles.meta}>
              No text threads found yet. Start a support chat and it’ll show up
              here.
            </Text>
          ) : null}
          ...
          {noResults ? (
            <Text style={styles.meta}>
              No text threads found for “{query.trim()}”.
            </Text>
          ) : null}
              ...
              {({ pressed }) => (
                <View style={styles.rowInner}>
                  ...
                  <View style={styles.rowBody}>
                    <Text style={styles.rowTitle}>{titleFor(thread)}</Text>
                    <Text style={styles.rowMeta}>{metaFor(thread)}</Text>
                    <Text style={styles.rowId}>#{thread.id}</Text>
                  </View>
                </View>
              )}
            </Pressable>
